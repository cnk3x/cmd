...
func TestWindowsCmd(t *testing.T) {
	s := Shell(`dir`).With(User(1000, 1000)).
		PostStart(func(c *Cmd) { t.Logf("command: %s", c.String()) }).
		LineRead(
...
func TestClash(t *testing.T) {
	s := CommandLine(`D:\services\clashpt\clash-windows-amd64.exe -d D:\services\clashpt\`).
		...
		With(User(1000, 1000)).
		PostStart(func(c *Cmd) { log.Printf("command1: %s", c.String()) }).
		Logger(LoggerOptions{Path: `D:\services\clashpt\clashpt.log`}).
		Run()
